...
%\allowdisplaybreaks %% Allows page breaks in align enviroments
...
\crefname{pluralequation}{equation}{equation}
...
\title{A Fully Coupled Discontinuous Galerkin Method for Diffusion Flames Embedded in a Low-Mach Solver Framework}
% \subtitle{}
\author[J. Gutiérrez-Jorquera]{Juan Francisco Gutiérrez Jorquera}%optionales Argument ist die Signatur,
\birthplace{Santiago de Chile}%Geburtsort, bei Dissertationen zwingend notwendig
\reviewer*[Erstreferent, Koreferent]{Prof. Dr.-Ing Martin Oberlack \and Prof. Dr. rer. nat. habil. Amsini Sadiki}
...
\cleardoublepage
\input{chapters_main/Computational_methodology}
\cleardoublepage
...
\label{bibref}
 ...
 \cleardoublepage
\appendix
\input{chapters_others/AppendixMatlabCode}
...
%TODO incluir algun tipo de introduccion similar a lo que dice en
%TODO Mathematical Models for Thermally Coupled Low Speed Flows

%TODO Como está exactamente definida la mass matrix?
...
%TODO debo  corregir en todos lados: En realidad para la deduccion de las ecuaciones de la mixture farction nunca se asume un cp constante. solo se asume que el cp de todos los componentes es el mismo. Las consecuencias siguen siendo las mismas, puesto que aunque °
%TODO Introduccion: Hacer una introduccion similar a la del paper de Aldo Hennin


%TODO Hablar de que dentro de varios casos estudiados en esta tesis, el tema de multiple solution branches se va repitiendo. (Es importante alguna cualidad del solver para poder calcular esto de forma adecuada?)
% Hablvar de porque es bacan el modelo de fernandez terrazo. Yo diria que representa una forma simple de representar fenomenos que solo serian posibles de predecir utilizando modelos quimicos muchos mas complejos, como por ejemplo el leakage. Entonces haciendo uso de una unica ecuacion quimica, es posible estudiar fenomenos complejos de combustionn, disminuyendo sustancialmente el costo computacional.  La idea es implementar un modelo versatil, pero a su vez que no brinde mayores complejidades tanto numericas como computacionales. 

%$$$: Todos creo que listos?
