...
\end{equation}
Here $\nu_{kj} = \nu''_{kj} -\nu'_{kj}$, and $\gls{rateProgress}$ is the rate of progress of the reaction $j$. It is usually modeled using Arrhenius-type expressions.\newline
...
\end{align}%
Here, the subscripts $F$ and $O$ refer to fuel and oxidizer, respectively. The parameter $\gls{preexpFact}$ corresponds to the pre-exponential factor, $\hat T_a$ is the activation temperature, and $a$ and $b$ are reaction orders. For a one-step reaction model, the reaction rate of the $k$-th component in\cref{eq:reacRateDef} is
\begin{equation}
